...
with dpg.window() as primary_window:
    with dpg.group(horizontal=True):
        start_pause_resume_button = dpg.add_button(
            label="Start", width=70, callback=start_stop_callback)
        reset_button = dpg.add_button(
            label="Reset", width=70, callback=reset_callback)
        dpg.hide_item(reset_button)
    progress_bar = dpg.add_progress_bar(
        default_value=0, width=-1, overlay="0%")

dpg.set_primary_window(primary_window, True)
dpg.create_viewport(width=400, height=300,
                    title="Progress Bar with Pause/Resume")
dpg.setup_dearpygui()
dpg.show_viewport()
dpg.start_dearpygui()
dpg.destroy_context()
